...
async function add() {
  // Get collection ID
  const collResult = await pool.query("SELECT id FROM collections WHERE slug = 'model-registry'");
  if (collResult.rows.length === 0) {
    console.log('Collection model-registry not found');
    await pool.end();
    return;
  }
  ...
  console.log('Added 8 categories to Model Registry');
  await pool.end();
}
add();
