...
async function add() {
  await pool.query(`
    ...
    VALUES ('gabriel-pastoral-voice', 'Gabriel (Pastoral Voice)', 'Gabriel (Pastoral Voice)', 'authority_and_identity', 'persona', TRUE, 5, 'Pastoral voice and guidance')
    ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name, display_name = EXCLUDED.display_name, updated_at = NOW()
  `);
  console.log('Added: Gabriel (Pastoral Voice)');
  await pool.end();
}
add();
